...
class CartItem extends \yii\db\ActiveRecord
{
    ...
    public static function getTotalQuantityForUser($currentUserId)
    {
        if (isGuest()){
            ...
            $sum = CartItem::findBySql(
                "SELECT SUM(quantity) 
                    FROM cart_items 
                    WHERE created_by = :userId", ['userId' => $currentUserId]
            )->scalar();
        }
        return $sum;
    }

    public static function getItemsForCurrentUser(?int $currentUserId)
    {
        if (isGuest()) {
            ...
            $cartItems = CartItem::findBySql(
                "SELECT 
                        ...
                        WHERE
                           c.created_by = :userId", ['userId' => $currentUserId]
            )
                ->asArray()
                ->all();
        } return $cartItems;
    ...
    public static function clearItemsForCurrentUser(?int $currentUserId)
    {
        ...
        if (isGuest()){
            ...
            $sum = CartItem::findBySql(
                "SELECT SUM(ci.quantity * p.price) 
                    ...
                    WHERE ci.created_by = :userId", ['userId' => $currentUserId]
            )->scalar();
        }
        return $sum;
    }
